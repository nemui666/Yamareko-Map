...
-(void) velocity:(float*)velocity ToHue:(float**)_hue count:(int)count{
    ...
    */
    ...
    float spanAbsAlt = fabs(maxAlt - minAlt);
    // 割合を求める
    float spanRituAlt = 180 / spanAbsAlt;
    for (int i=0;i<count;i++){
        // 相対高さ
        float curVelo = velocity[i] - minAlt;
        float tmpHue = 180 - curVelo * spanRituAlt;
        (*_hue)[i] = tmpHue/360;
        
        /*
        if (spanAlt*3 <= curVelo && spanAlt*4 > curVelo) {
            (*_hue)[i] = 0;
        } else if (spanAlt*2 <= curVelo && spanAlt*3 > curVelo) {
            (*_hue)[i] = 0.1;
        } else if (spanAlt <= curVelo && spanAlt*2 > curVelo) {
            (*_hue)[i] = 0.2;
        } else if (0 <= curVelo && spanAlt*1 > curVelo) {
            (*_hue)[i] = 0.3;
        } else {
            (*_hue)[i] = 0;
        }
